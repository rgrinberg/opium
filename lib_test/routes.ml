...
let simple_route2 () =
  ...
  Alcotest.(
    check (option params) "" m (Some [("format", "json"); ("name", "bar")]))
...
let splat_route1 () =
  ...
  Alcotest.(
    check (option matches_t) "matches"
...
let test_match_no_param () =
  ...
  match (m1, m2) with Some _, None -> () | _, _ -> Alcotest.fail "bad match"

let test_empty_route () =
  let r = Route.of_string "/" in
  let m s = match Route.match_url r s with None -> false | Some _ -> true in
