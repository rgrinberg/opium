...
module Env = struct
  let key : Route.matches Request.Context.key =
    Request.Context.Key.create ("path_params", Route.sexp_of_matches)
  ;;
end

let splat req =
  Request.Context.find_exn Env.key req.Request.env |> fun route -> route.Route.splat
;;

(* not param_exn since if the endpoint was selected it's likely that the parameter is
   already there *)
let param req param =
  let { Route.params; _ } = Request.Context.find_exn Env.key req.Request.env in
...
let m endpoints =
  let filter default req =
    ...
    | Some (endpoint, params) ->
      let env_with_params = Request.Context.add Env.key params req.Request.env in
