...
module Make (IO : Cohttp_lwt.S.IO) = struct
  ...
  module Response = struct
    type t =
      ...
      ; action: (IO.ic -> IO.oc -> unit IO.t) option
      ; env: Hmap0.t }
    [@@deriving fields, sexp_of]
    ...
    let of_action ?(env = Hmap0.empty) ?headers ?(code = `OK) action =
      {env; code; headers= default_header headers; body= `Empty; action = Some action}
  end
